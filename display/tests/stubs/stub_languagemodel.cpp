/***************************************************************************
**
** Copyright (C) 2010 Nokia Corporation and/or its subsidiary(-ies).
** All rights reserved.
** Contact: Karoliina T. Salminen <karoliina.t.salminen@nokia.com>
**
** This file is part of duicontrolpanel-datetimeapplet.
**
**
** This program is free software; you can redistribute it and/or
** modify it under the terms of the GNU Lesser General Public
** License version 2.1 as published by the Free Software Foundation
** and appearing in the file LICENSE.LGPL included in the packaging
** of this file.
**
****************************************************************************/
/* -*- Mode: C; indent-tabs-mode: s; c-basic-offset: 4; tab-width: 4 -*- */
/* vim:set et ai sw=4 ts=4 sts=4: tw=80 cino="(0,W2s,i2s,t0,l1,:0" */

#include "languagemodel.h"
#include <QStringList>
#include <QVariant>
#include <QDebug>

#include <MAbstractItemModel>

LangData::LangData(const QString& lang)
{
    code = lang;
    name = lang;
};

LanguageModel& LanguageModel::instance()
{
    static LanguageModel model;
    return model;
}


LanguageModel::LanguageModel(QObject *parent) :
    MAbstractItemModel(parent)
{
}

LanguageModel::~LanguageModel()
{
}

QModelIndex LanguageModel::getIndex(const QString &) const
{
    QModelIndex idx = index(0, 0);
    return idx;
}

void LanguageModel::populateModel()
{
}

void LanguageModel::updateData(const QModelIndex &first, const QModelIndex &last)
{
    emit dataChanged(first, last);
}

int LanguageModel::groupCount() const
{
    return 1;
}
int LanguageModel::rowCountInGroup(int) const
{
   return 1;
}

QString LanguageModel::groupTitle(int) const
{
    return QString("GrT");
}

QVariant LanguageModel::itemData(int, int, int role) const
{
    QStringList list;
    switch (role)
    {
        case DisplayRole:
            list << "en_US" << "USA";
            qWarning() << "TTTT" << list;
            return QVariant(list);
        default:
            return QVariant("USA");
    }
}

void LanguageModel::retranslateModel()
{

}

void LanguageModel::doRetranslateModel()
{

}
