...
void Ut_DcpDisplayLangConf::testAvailableDisplayLanguagesIdConvert_data()
{
 ...
 /**
 * Test language file name filtering
 */
void Ut_DcpDisplayLangConf::testAvailableDisplayLanguagesFilter()
{
    QFETCH(QStringList, fileNames);
    QFETCH(QStringList, localeIds);

    setQDirStubContent(fileNames);
    setDefaultStubMLocaleId("en");
    QStringList result =
        DcpDisplayLangConf::instance()->availableDisplayLanguages();
    QCOMPARE(result, localeIds);
}

void Ut_DcpDisplayLangConf::testAvailableDisplayLanguagesFilter_data()
{
    QTest::addColumn<QStringList>("fileNames");
    QTest::addColumn<QStringList>("localeIds");

    QStringList qDirContent;
    qDirContent << "de" << "fi_FI" << "de.dpkg-old" << "es_419" << "cs";

    QStringList latinExpected;
    latinExpected << "cs" << "de" << "es_419" << "fi_FI";
    QTest::newRow("latin script") << qDirContent << latinExpected;
}
