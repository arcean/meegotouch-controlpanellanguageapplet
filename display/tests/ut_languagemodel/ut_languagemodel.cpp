...
void Ut_LanguageModel::testItemData()
{
    ...
    QModelIndex idx = model.getIndex("hu_HUFull");

    QCOMPARE(model.itemData(0, 0, LanguageModel::DisplayRole).toStringList(), QStringList() << "hu_HU" << "hu_HUFull");
    QCOMPARE(model.itemData(0, 0, Qt::UserRole+1).toString(), QString("hu_HUFull"));
}
...
void Ut_LanguageModel::testGetIndex()
{
    ...
    LanguageModel &model = LanguageModel::instance();
    QModelIndex idx = model.getIndex("ize");

    Q_ASSERT(!idx.isValid());

    idx = model.getIndex("hu_HUFull");
    Q_ASSERT(idx.isValid());
