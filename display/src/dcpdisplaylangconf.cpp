...
namespace GConfKey
{
    ...
    const QString UserSelectedNumber("/meegotouch/i18n/userselectednumberformat");
}

const char *RegionKeys[] = {
    "region", "lc_monetary", "lc_collate", "lc_time", 
#ifndef UNSUPPORT_ARABIC
    "lc_numeric",
#endif    
    // terminate with 0!
    0
...
DcpDisplayLangConf::ScriptClass
DcpDisplayLangConf::scriptClass()
{
    Q_D(DcpDisplayLangConf);
    return d->scriptClass();
}
...
#ifdef UNSUPPORT_ARABIC    
    // exception for Arabic regions
    MGConfItem gconfNum(GConfKey::RegionRoot + "/lc_numeric");
    if (localeID.startsWith("ar")) {
        gconfNum.set("en");
    } else {
        gconfNum.set(localeID);
    }
#endif    
    LEAVE_FUNCTION();
}
...
QString
DcpDisplayLangConfPrivate::getNewRegionId(const QString &langLocaleId)
{
    QString region = langLocaleId;

    // Restore the original ID, to be able to open the correct data file
    if (region == "en_GB") {
        region = "en";
    }

    QFile langData(SupportedLanguagesDir + "/" + region);
    if (!langData.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "Unable to open data file???" << SupportedLanguagesDir + langLocaleId;
        return QString();
    }

    QStringList result;
    QTextStream inStream(&langData);
    QString defaultRegion;
    while (!inStream.atEnd()) {
        QString line = inStream.readLine();
        QStringList fields = line.split("=", QString::SkipEmptyParts);
        if (fields[0].contains("default_region_locale")) {
            fields.removeFirst();
            defaultRegion = fields.join("=");
            break;
        }
    }
    RegionConfReader conf;
    if (!defaultRegion.isEmpty()) {
        defaultRegion = defaultRegion.simplified();
        // Check whether the region is supported
        if (conf.availableRegions().contains(defaultRegion)) {
            return defaultRegion;
        }
        qDebug() << "Default region not supported";
    }
...
DcpDisplayLangConfPrivate::updateRegionFormat(const QString &langLocaleId)
{
    ...
    MGConfItem userSelectedNumber(GConfKey::UserSelectedNumber);

    bool isRegionUserSelected;
    bool isNumberUserSelected;

    isRegionUserSelected = userSelectedRegion.value().toBool();
    isNumberUserSelected = userSelectedNumber.value(QVariant(false)).toBool();

    QString currentNumberFormat;
    QString currentMonetaryFormat;

    MGConfItem currentNumberFormatGC("/meegotouch/i18n/lc_numeric");
    currentNumberFormat = currentNumberFormatGC.value().toString();
    currentMonetaryFormat = MGConfItem("/meegotouch/i18n/lc_monetary").value().toString();

    QString newRegionId;

    if (isRegionUserSelected && !isNumberUserSelected && (currentNumberFormat != currentMonetaryFormat)) {
        // Check the new regionformat, and if that has the 'ar' prefix, change the numberformat.
        newRegionId = getNewRegionId(langLocaleId);
        if (newRegionId.startsWith("ar")) {
            currentNumberFormatGC.set(currentMonetaryFormat);
        }
    }
    ...
    if (!userSelectedRegion.value().toBool()) {
        ...
        if (newRegionId.isEmpty()) {
            newRegionId = getNewRegionId(langLocaleId);
        }
        if (newRegionId.isEmpty()) {
            return;
        }
        setRegionFormat(newRegionId);
    }
...
DcpDisplayLangConfPrivate::scriptClass(const QString &localeId)
{
    ...
    if (s == "Latn") return DcpDisplayLangConf::ScriptClassLatin;
    if (s == "Arab") return DcpDisplayLangConf::ScriptClassArabic;
    if (s == "Cyrl") return DcpDisplayLangConf::ScriptClassCyrillic;


    return DcpDisplayLangConf::ScriptClassOther;
}
...
QStringList DcpDisplayLangConfPrivate::availableDisplayLanguages()
{
    ...
    foreach (QString fileName, supportedLangDir.entryList()) {
        ...
        QRegExp localeIdPattern("^[a-z]+(_\\w+)?$");
        if (!localeIdPattern.exactMatch(fileName)) {
            qDebug() << "ignoring" << fileName;
            continue;
        }
        ...
        DcpDisplayLangConf::ScriptClass script = scriptClass(localeId);
        scriptIdListMap[script].append(localeId);
        qDebug() << "have locale" << localeId << "script" << script;
    }

    DcpDisplayLangConf::ScriptClass currentScript = scriptClass();
    ...
    for(int i = DcpDisplayLangConf::ScriptClassLatin; 
        i < DcpDisplayLangConf::ScriptClassLast; ++i) {
        if (i != currentScript) {
