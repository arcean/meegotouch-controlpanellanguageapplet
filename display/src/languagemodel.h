...
class LanguageModel : public MAbstractItemModel {
    ...
    public:
        ...
        void populateModel();
        void retranslateModel();
    private slots:
        void doRetranslateModel();
    private:
        LanguageModel(QObject *parent=0);

        QList<LangData> m_Languages;
        QStringList m_GroupLabels;
        QMap<int, QList<LangData> > m_GroupMap;
        // language -> (group, row) index vector
        QMap<QString, QVector<int> > m_LanguageIndexMap;
        DcpDisplayLangConf::ScriptClass m_SortingScript; // script the list is sorted by
